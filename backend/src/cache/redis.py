...
async def get_redis_value(
    ...
    """Retrieve a value from Redis and deserialize it if it exists."""
    if serializer is None:
        serializer = PickleSerializer()

    cached_value = await cache.get(key)
    return None if cached_value is None else serializer.deserialize(cached_value)
...
def cached(
    ...
    """
    Caches the functions return value into a key generated with module_name,
...
async def clear_cache(
    ...
    **kwargs: Any,
    ...
    Keyword Arguments:
    - namespace (str, optional): A string indicating the namespace for the cache.
    Defaults to "main".
    """
