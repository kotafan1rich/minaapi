...
from src.admins.schemas import ResponseAdmin, ResponseAdminList
...
async def create_admin(user_id: int, db_session=Depends(get_db)) -> ResponseAdmin:
    admin_dao = AdminDAO(db_session=db_session)
    created_admin = await admin_dao.create_admin(user_id=user_id)
    if created_admin:
        return ResponseAdmin.model_validate(created_admin)


@admin_router.delete("/delete_admin")
async def delete_admin(id: int, db_session=Depends(get_db)) -> ResponseAdmin:
    admin_dao = AdminDAO(db_session=db_session)
    ...
    return ResponseAdmin.model_validate(deleted_admin)


@admin_router.get("/get_admin")
async def get_admin(id: int, db_session=Depends(get_db)) -> ResponseAdmin:
    admin_dao = AdminDAO(db_session=db_session)
    ...
    return ResponseAdmin.model_validate(admin)
...
async def get_admins_list(
    ...
    return ResponseAdminList(
        results=[ResponseAdmin.model_validate(admin) for admin in admins_list]
    )
