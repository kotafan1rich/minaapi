...
from src.app_settings.schemas import ResponseSetting, ResponseSettingsList
...
async def create_setting(
    ...
    if created_setting:
        return ResponseSetting.model_validate(created_setting)
...
async def get_settings_list(
    ...
    return ResponseSettingsList(
        results=[ResponseSetting.model_validate(setting) for setting in settings_list]
    )


@app_setting_router.delete("/delete_setting")
async def delete_setting(id: int, db_session=Depends(get_db)) -> ResponseSetting:
    settings_dao = AppSettingDAO(db_session=db_session)
    deleted_setting = await settings_dao.delete_param(id=id)
    if deleted_setting:
        return ResponseSetting.model_validate(deleted_setting)


@app_setting_router.get("/get_setting")
async def get_setting(key: str, db_session=Depends(get_db)) -> ResponseSetting:
    setting_dao = AppSettingDAO(db_session=db_session)
    setting = await setting_dao.get_param(key=key)
    if setting:
        return ResponseSetting.model_validate(setting)
...
async def update_setting(
    ...
    if updated_setting:
        return ResponseSetting.model_validate(updated_setting)
