...
class OrderDAO(BaseDAO):
    async def create_order(self, user_id: int, **kwargs) -> Order | None:
    ...
    async def update_order(self, id: int, **kwargs) -> Order | None:
        async with self.db_session.begin():
    ...
    async def delete_order(self, id: int) -> Order | None:
        async with self.db_session.begin():
            query = delete(Order).where(Order.id == id).returning(Order)
            deleted_order = await self.db_session.execute(query)
            await self.db_session.commit()
            return deleted_order.scalar_one_or_none()
