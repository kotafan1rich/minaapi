...
async def create_order(data: RequestOrder, db_session=Depends(get_db)) -> ResponseOrder:
    order_dao = OrderDAO(db_session=db_session)
    created_order = await order_dao.create_order(**data.model_dump())
    if created_order:
        return ResponseOrder.model_validate(created_order)
...
async def get_order_list(
    ...
    return ListOrders(
        results=[ResponseOrder.model_validate(order) for order in orders_list]
    )
...
async def get_active_orders(db_session=Depends(get_db)) -> ListOrders:
    ...
    if result:
        return ListOrders(
            results=[ResponseOrder.model_validate(order) for order in result]
        )
    else:
        raise HTTPException(status_code=404, detail="Orders not found")
...
async def get_completed_orders(db_session=Depends(get_db)) -> ListOrders:
    ...
    if result:
        return ListOrders(
            result=[ResponseOrder.model_validate(order) for order in result]
        )
    else:
        raise HTTPException(status_code=404, detail="Orders not found")
...
async def delete_order(id: int, db_session=Depends(get_db)) -> ResponseOrder:
    order_dao = OrderDAO(db_session=db_session)
